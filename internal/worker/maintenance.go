...
type MaintenanceRepository interface {
	...
	DeleteExpiredIdempotencyBatch(context.Context, time.Time) (int64, error)
...
func (m *Maintenance) CleanupIdempotency(ctx context.Context, _ domain.WorkerJob) error {
	_, err := m.repo.DeleteExpiredIdempotencyBatch(ctx, m.now.Now())
	return err
}
...
func (m *Maintenance) ExpireEscalations(ctx context.Context, _ domain.WorkerJob) error {
	...
	return m.repo.WithinTx(ctx, func(tx *sql.Tx) error {
		...
		for _, current := range items {
			...
			if err := m.repo.UpdateEscalation(ctx, tx, updated, current.Version); err != nil {
