...
type Repository interface {
	...
	PersistProviderSuspension(context.Context, domain.Provider, int64) error
}
...
func (s *Service) Suspend(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Provider, error) {
	...
	if current.Version != expectedVersion {
		return domain.Provider{}, fmt.Errorf("provider version conflict")
	}
	updated, err := current.Suspend(now)
	if err != nil {
		return domain.Provider{}, err
	}
	if err := s.repo.PersistProviderSuspension(ctx, updated, current.Version); err != nil {
		return domain.Provider{}, err
	}
	err = s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
		if err := s.audit.Record(ctx, tx, actor, "provider", id, "suspend", "success", map[string]any{"status": updated.AccreditationStatus}, now); err != nil {
			return err
		}
