...
func (s *Service) ReviewClaim(ctx context.Context, actor domain.Actor, claimID string, approve bool, reason string, expectedVersion int64) (domain.Claim, error) {
	...
	now := s.now.Now()
	if approve { current,err:=s.repo.ClaimByID(ctx,nil,claimID,actor.DistrictID);if err!=nil{return domain.Claim{},mapBenefitError("review claim",err)};if current.Version!=expectedVersion{return domain.Claim{},apperr.Conflict("claim version conflict")};updated,err:=current.Approve(actor.UserID,now);if err!=nil{return domain.Claim{},mapBenefitError("review claim",err)};if err:=s.repo.PersistClaimReview(ctx,updated,current.Version);err!=nil{return domain.Claim{},mapBenefitError("review claim",err)};if err:=s.repo.WithinTx(ctx,func(tx *sql.Tx)error{return s.audit.Record(ctx,tx,actor,"claim",claimID,"review","success",map[string]any{"status":updated.Status},now)});err!=nil{return domain.Claim{},mapBenefitError("review claim",err)};return updated,nil }
