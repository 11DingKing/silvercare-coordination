...
func (s *Service) Activate(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Authorization, error) {
	...
	now := s.now.Now()
	current, err := s.repo.AuthorizationByID(ctx, nil, id, actor.DistrictID)
	if err != nil {
		return domain.Authorization{}, mapBenefitError("load authorization", err)
	}
	plan, err := s.repo.PlanByID(ctx, nil, current.PlanID)
	if err != nil {
		return domain.Authorization{}, mapBenefitError("load plan", err)
	}
	if current.Version != expectedVersion {
		return domain.Authorization{}, mapBenefitError("activate authorization", fmt.Errorf("authorization version conflict"))
	}
