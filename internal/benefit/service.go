...
type Repository interface {
	...
	ReserveAuthorizationBudget(context.Context, string, int64, time.Time) error
...
func (s *Service) Authorize(ctx context.Context, actor domain.Actor, planID, serviceCode string, units int, unitPrice int64, startsAt, endsAt time.Time) (domain.Authorization, error) {
	...
	if err := s.repo.ReserveAuthorizationBudget(ctx, actor.DistrictID, a.ReservedCents, now); err != nil {
		return domain.Authorization{}, mapBenefitError("reserve authorization budget", err)
	}
