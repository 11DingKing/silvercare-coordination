...
type Repository interface {
	...
	PersistResidentConsent(context.Context, domain.Resident, int64) error
...
func (s *Service) GrantConsent(ctx context.Context, actor domain.Actor, residentID string, until time.Time, expectedVersion int64) (domain.Resident, error) {
	...
	current, err := s.repo.ResidentByID(ctx, nil, residentID, actor.DistrictID)
	if err != nil {
		return domain.Resident{}, mapWriteError("load resident consent", err)
	}
	if current.Version != expectedVersion {
		return domain.Resident{}, mapWriteError("grant resident consent", fmt.Errorf("resident version conflict"))
	}
	updated, err = current.GrantConsent(until, now)
	if err != nil {
		return domain.Resident{}, mapWriteError("grant resident consent", err)
	}
	if err := s.repo.PersistResidentConsent(ctx, updated, current.Version); err != nil {
		return domain.Resident{}, mapWriteError("persist resident consent", err)
	}
	err = s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
