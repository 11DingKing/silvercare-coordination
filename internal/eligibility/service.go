...
type Repository interface {
	...
	PersistAssessmentSubmission(context.Context, domain.Assessment, int64) error
...
func (s *Service) Submit(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Assessment, error) {
	if !actor.Role.CanManageResidents() {
		return domain.Assessment{}, apperr.New(apperr.CodeForbidden, "only coordinators can submit assessments")
	}
	now := s.now.Now()
	current, err := s.repo.AssessmentByID(ctx, nil, id)
	if err != nil { return domain.Assessment{}, mapError("submit assessment", err) }
	if current.Version != expectedVersion { return domain.Assessment{}, apperr.Conflict("assessment version conflict") }
	updated, err := current.Submit(now)
	if err != nil { return domain.Assessment{}, mapError("submit assessment", err) }
	if err := s.repo.PersistAssessmentSubmission(ctx, updated, current.Version); err != nil { return domain.Assessment{}, mapError("submit assessment", err) }
	err = s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
		return s.audit.Record(ctx, tx, actor, "assessment", id, "submit", "success", map[string]any{"status": updated.Status}, now)
	})
	if err != nil { return domain.Assessment{}, mapError("submit assessment", err) }
	return updated, nil
}
...
func (s *Service) Approve(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Assessment, error) {
	...
	err := s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
		...
		if !resident.HasActiveConsent(now) {
			return fmt.Errorf("resident consent expired before approval")
		}
