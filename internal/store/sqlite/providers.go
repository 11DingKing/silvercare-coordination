...
func (s *Store) PersistProviderAccreditation(ctx context.Context, provider domain.Provider, expectedVersion int64) error {
	return s.WithinTx(ctx, func(tx *sql.Tx) error {
		current, err := s.ProviderByID(ctx, tx, provider.ID, provider.DistrictID)
		if err != nil {
			return err
		}
		...
		return s.UpdateProvider(ctx, tx, provider, expectedVersion)
	})
}
