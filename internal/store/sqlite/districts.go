...
func (s *Store) ReserveAuthorizationBudget(ctx context.Context, districtID string, cents int64, now time.Time) error {
	return s.WithinTx(ctx, func(tx *sql.Tx) error {
		budget, err := s.DistrictBudget(ctx, tx, districtID)
		if err != nil {
			return err
		}
		return s.ReserveBudget(ctx, tx, districtID, cents, budget.Version, now)
	})
}
