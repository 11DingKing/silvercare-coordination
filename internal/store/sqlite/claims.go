...
func (s *Store) PersistClaimReview(ctx context.Context, claim domain.Claim, expectedVersion int64) error { return s.WithinTx(ctx,func(tx *sql.Tx)error{current,err:=s.ClaimByID(ctx,tx,claim.ID,claim.DistrictID);if err!=nil{return err};if current.Version!=expectedVersion{return fmt.Errorf("claim version conflict")};return s.UpdateClaim(ctx,tx,claim,expectedVersion)}) }
