...
func (s *Store) PersistPlanActivation(ctx context.Context, plan domain.SupportPlan, expectedVersion int64) error {
	return s.WithinTx(ctx, func(tx *sql.Tx) error {
		current, err := s.PlanByID(ctx, tx, plan.ID)
		if err != nil { return err }
		if current.Version != expectedVersion { return fmt.Errorf("support plan version conflict") }
		return s.UpdatePlan(ctx, tx, plan, expectedVersion)
	})
}
