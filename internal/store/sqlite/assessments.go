...
func (s *Store) PersistAssessmentSubmission(ctx context.Context, assessment domain.Assessment, expectedVersion int64) error {
	return s.WithinTx(ctx, func(tx *sql.Tx) error {
		current, err := s.AssessmentByID(ctx, tx, assessment.ID)
		if err != nil { return err }
		if current.Version != expectedVersion { return fmt.Errorf("assessment version conflict") }
