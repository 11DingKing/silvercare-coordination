...
func (s *Store) PersistEscalationResolution(ctx context.Context, escalation domain.Escalation, expectedVersion int64) error {
	return s.WithinTx(ctx,func(tx *sql.Tx)error{current,err:=s.EscalationByID(ctx,tx,escalation.ID,escalation.DistrictID);if err!=nil{return err};if current.Version!=expectedVersion{return fmt.Errorf("escalation version conflict")};return s.UpdateEscalation(ctx,tx,escalation,expectedVersion)})
}
