...
func (s *Service) Complete(ctx context.Context, actor domain.Actor, visitID string, evidence map[string]string, at time.Time, expectedVersion int64) (domain.Visit, error) {
	...
	return s.change(ctx, actor, visitID, expectedVersion, "complete", func(tx *sql.Tx, current domain.Visit, now time.Time) (domain.Visit, error) {
		updated, err := current.Complete(actor.UserID, evidence, at, now)
		if err != nil {
			return domain.Visit{}, err
		}
		a, err := s.repo.AuthorizationByID(ctx, tx, current.AuthorizationID, actor.DistrictID)
		if err != nil {
			return domain.Visit{}, err
		}
		...
		if err := s.repo.UpdateAuthorization(ctx, tx, consumed, a.Version); err != nil {
			return domain.Visit{}, err
		}
		return updated, nil
	})
