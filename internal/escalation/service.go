...
type Repository interface {
	...
	PersistEscalationAcknowledgement(context.Context, domain.Escalation, int64) error
}
...
func (s *Service) Acknowledge(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Escalation, error) {
	...
	now := s.now.Now()
	current, err := s.repo.EscalationByID(ctx, nil, id, actor.DistrictID)
	if err != nil {
		return domain.Escalation{}, mapEscalationError("load escalation", err)
	}
	if current.Version != expectedVersion {
		return domain.Escalation{}, mapEscalationError("acknowledge escalation", fmt.Errorf("escalation version conflict"))
	}
	updated, err := current.Acknowledge(actor.UserID, now)
	if err != nil {
		return domain.Escalation{}, mapEscalationError("acknowledge escalation", err)
	}
	if err := s.repo.PersistEscalationAcknowledgement(ctx, updated, current.Version); err != nil {
		return domain.Escalation{}, mapEscalationError("persist escalation acknowledgement", err)
	}
	err = s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
		if err := s.audit.Record(ctx, tx, actor, "escalation", id, "acknowledge", "success", map[string]any{"status": updated.Status}, now); err != nil {
			return err
		}
		return s.events.Enqueue(ctx, tx, actor.DistrictID, "escalation.acknowledge", "escalation", id, map[string]any{"status": updated.Status}, now)
	})
	if err != nil {
		return domain.Escalation{}, mapEscalationError("acknowledge escalation", err)
	}
...
func (s *Service) Resolve(ctx context.Context, actor domain.Actor, id string, expectedVersion int64) (domain.Escalation, error) {
	...
	return s.change(ctx, actor, id, expectedVersion, "resolve", func(current domain.Escalation) (domain.Escalation, error) {
		return current.Resolve(actor.UserID, s.now.Now())
	})
}
