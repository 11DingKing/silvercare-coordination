...
type Repository interface {
	...
	PersistResourceReturn(context.Context, domain.Resource, int64) error
...
func (s *Service) Return(ctx context.Context, actor domain.Actor, resourceID string, quarantine bool, expectedVersion int64) (domain.Resource, error) {
	...
	if err != nil {
		return domain.Resource{}, mapResourceError("load returned resource", err)
	}
	if current.Version != expectedVersion {
		return domain.Resource{}, mapResourceError("return resource", fmt.Errorf("resource version conflict"))
	}
	updated, err := current.Return(quarantine, now)
	if err != nil {
		return domain.Resource{}, mapResourceError("return resource", err)
	}
	if err := s.repo.PersistResourceReturn(ctx, updated, current.Version); err != nil {
		return domain.Resource{}, mapResourceError("persist resource return", err)
	}
	err = s.repo.WithinTx(ctx, func(tx *sql.Tx) error {
